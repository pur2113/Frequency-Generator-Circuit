...
; Giving names for the internal addresses of different ports of 8255
...
; Giving names to the different buttons hexcodes present on keypad..
...
; Initializing the control register of 8255 accordingly
...
; if key is pressed in first column
mov     al, 06H
mov     bl, al
out     portC, al
in      al, portC
...
; if key is pressed in second column
mov     al, 05H
mov     bl, al
out     portC, al
in      al, portC
...
; if key is pressed in third column
mov     al, 03H
mov     bl, al
out     portC, al
in      al, portC
...
; we will do OR of al and bl to store the hexcode corresponding to a particular button pressed
...
; Calculated frequency present in count variable
; Storing count

mov     al, 00H
out     portC, al

; Wait for GEN key to be released
...
; now we will be selecting the wave form whose button is pressed maximum number of times
...
mov [si],al
inc si
loop x99			;loop again and again to change values of sine table according to given input..
...
mov bl,15
mul bl
mov vfac,al			;vfac now is having the max amplitude
...
; If a key is pressed, we find out which one is pressed:
...
; Procedure to wait for GEN key to be released
